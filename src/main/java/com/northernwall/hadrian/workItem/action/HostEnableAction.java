...
public class HostEnableAction extends Action {
    ...
    private final static Logger LOGGER = LoggerFactory.getLogger(HostEnableAction.class);

    @Override
    public Result process(WorkItem workItem) {
        return Result.success;
        //return enableVips(workItem);
    }
    ...
    protected void recordAudit(WorkItem workItem, Result result, List<Vip> successVips, Vip failedVip) {
        ...
        if (successVips == null || successVips.isEmpty()) {
            if (failedVip == null) {
                output = "No VIPs to enable";
            } else {
                output = "Failed to enable VIP " + failedVip.getDns();
            }
        } else {
            output = "VIPs successfully enabled:\n";
