 ...
 * @author Richard
 */
public class VipAddHostHandler extends BasicHandler {

    private final AccessHelper accessHelper;
    private final WorkItemProcessor workItemProcessor;

    public VipAddHostHandler(DataAccess dataAccess, Gson gson, AccessHelper accessHelper, WorkItemProcessor workItemProcessor) {
    ...
    public void handle(String target, Request request, HttpServletRequest httpRequest, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (vip.getDisabledHosts().contains(data.hostname)) {
            vip.getDisabledHosts().remove(data.hostname);
